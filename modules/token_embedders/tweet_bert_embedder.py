...
from transformers import XLNetConfig, RobertaConfig, RobertaModel

from allennlp.modules.token_embedders.token_embedder import TokenEmbedder
from allennlp.nn.util import batched_index_select


@TokenEmbedder.register("tweet_bert")
class TweetBertEmbedder(TokenEmbedder):
    """
    ...
    def __init__(self, model_path: str, max_length: int = None) -> None:
        ...
        BERTweet = RobertaModel.from_pretrained(
            f"{model_path}/model.bin",
            config=config
        )
        self.transformer_model = BERTweet
