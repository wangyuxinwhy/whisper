...
@Tokenizer.register("pretrained_transformer_zh")
class PretrainedTransformerZhTokenizer(Tokenizer):

    ZH_TOKENIZER_MAPPING = {"albert": BertTokenizer}
    ...
    def __init__(
        ...
        for name, map_tokenizer in self.ZH_TOKENIZER_MAPPING.items():
            if name in model_name and "zh" in model_name:
                _tokenizer = map_tokenizer
                break

        self.tokenizer = _tokenizer.from_pretrained(
        ...
        tokenizer_with_special_tokens = _tokenizer.from_pretrained(
    ...
    def num_special_tokens_for_sequence(self) -> int:
        return len(self.single_sequence_start_tokens) + len(
            self.single_sequence_end_tokens
        )
